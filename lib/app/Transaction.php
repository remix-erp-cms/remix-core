...
class Transaction extends Model
{
    ...
    {
        return $this->belongsTo(\App\Stock::class, 'stock_id');
    }
    ...
    {
        return $this->hasMany(\App\Accountant::class, 'transaction_id');
    }
    ...
    public static function getPaymentStatus($transaction)
    {
        ...
        if (in_array($payment_status, ['payment_paid', 'payment_pending', 'payment_due']) && !empty($transaction->pay_term_number) && !empty($transaction->pay_term_type)) {
            ...
            $now = \Carbon::now();
            if ($now->gt($due_date)) {
                $payment_status = $payment_status == 'payment_due' ? 'overdue' : 'partial-overdue';
            }
        }
