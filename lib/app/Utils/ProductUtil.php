...
class ProductUtil extends Util
{
    ...
    public function decreaseProductStock($product_id, $stock_id, $quantity = 0)
    {
...
//            ];
//
//            $result = DB::table('stock_products')
//                ->where('product_id', $product_id)
//                ->where('stock_id', $stock_id)
//                ->update($data);
//
...
//           }
     ...
     */
    public function calculateInvoiceTotal($products, $tax_id, $discount = null, $modifier_price = null, $shippingCharge = 0)
    {
        if (empty($products)) {
            return false;
        }
        ...
        $output = [
            ...
            'tax' => 0,
            'discount' => 0,
            'shipping_charges' => $this->num_uf($shippingCharge),
            'final_total' => 0,
            'profit_total' => 0
        ];
        ...
        //Calculate total
        $output['final_total'] = $output['total_before_tax'] + $output['tax'] + $output['shipping_charges'] - $output['discount'];

        return $output;
    }

    public function calculatePurchaseReturnTotal($products, $tax_id, $discount = null, $modifier_price = null)
    {
        if (empty($products)) {
            return false;
        }
        ...
        foreach ($products as $product) {
            ...
            $output['total_before_tax'] += $purchase_price * $quantity;
            $output['profit_total'] += 0;
        }
    ...
    public function calculateTotalSellSold($products, $tax_id, $discount = null)
    {
        if (empty($products)) {
            return false;
        }
        ...
        foreach ($products as $product) {
            $unit_price = $this->num_uf($product['unit_price']);
            $purchase_price = $this->num_uf($product['purchase_price']);
            $quantity = $this->num_uf($product['quantity_sold']);
    ...
    public function calculateTotalPurchaseSold($products, $tax_id, $discount = null)
    {
        if (empty($products)) {
            return false;
        }
     ...
     */
    public function generateProductSku($string, $business_id, $sku_prefix = "SP")
    {
        if (!$business_id) {
            $business_id = request()->session()->get('user.business_id');
        }

        $skuPrefix = Business::where('id', $business_id)->value('sku_prefix');

        if ($sku_prefix) {
            $skuPrefix = $sku_prefix;
        }

        return $skuPrefix . str_pad($string, 4, '0', STR_PAD_LEFT);
    }
