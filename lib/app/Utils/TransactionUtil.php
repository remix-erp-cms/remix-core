...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $profit_total = $invoice_total['profit_total'];
        ...
        $transaction = Transaction::create([
            ...
            'type' => $input['type'],
            'status' => !empty($input['status']) ? $input['status'] : "ordered",
            'payment_status' => !empty($input['payment_status']) ? $input['payment_status'] : "payment_pending",
            'res_order_status' => !empty($input['res_order_status']) ? $input['res_order_status'] : null,
            ...
            'discount_amount' => !empty($input['discount_amount']) ? $input['discount_amount'] : null,
            'tax_amount' => $invoice_total['tax'],
            'final_total' => $final_total,
            'final_profit' => $profit_total,
            ...
            'commission_agent' => isset($input['commission_agent']) ? $input['commission_agent'] : 0,
            'is_quotation' => isset($input['is_quotation']) ? $input['is_quotation'] : 0,
            'delivery_company_id' => isset($input['delivery_company_id']) ? $input['delivery_company_id'] : null,
            'shipping_type' => isset($input['shipping_type']) ? $input['shipping_type'] : null,
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $profit_total = $uf_data ? $this->num_uf($invoice_total['profit_total']) : $invoice_total['profit_total'];
        ...
        if (isset($input["status"]) && $input["status"]) {
            $update_date["status"] = $input["status"];
        }
        ...
        $isDirect = false)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                if ($stock_id) {
                    $stock_variable = StockProduct::where('stock_id', $stock_id)
                        ->where('product_id', $product['product_id'])
                        ->first();

                    if ($stock_variable->id) {
                        $stock_product_id = $stock_variable->id;
                    }

                    if (isset($stock_variable->quantity)
                        && $stock_variable->quantity >= $uf_quantity) {
                        $res_order_status = "enough";
                    } else {
                        $qty_defi = $uf_quantity - $stock_variable->quantity;
...
//        }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //Subtotal
        $output['subtotal_label'] = $il->sub_total_label . ': VND';
        ...
        $output['tax_label'] .= ': ';
        if (isset($tax->amount) && $tax->amount) {
            $output['tax_label'] .= ': ' . $tax->amount . "%";
        }
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['total_label'] = $invoice_layout->total_label . ': VND';
            $output['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
        }
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        $status = 'payment_due';
        if ($final_amount == 0 || $total_paid == 0) {
            $status = 'payment_due';
    ...
    public function updateOrderStatus($transaction_id, $order_status = "process")
    {
        Transaction::where('id', $transaction_id)
            ->update(['res_order_status' => $order_status]);
