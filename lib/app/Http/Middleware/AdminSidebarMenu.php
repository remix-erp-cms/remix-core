            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.request_price.view')) {
                            $sub->url(
                                action('ProductController@showCustomerPrices'),
                                "DS Báo giá",
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'products' && request()->segment(2) == 'list-customer-prices']
                            );
                        }
                        if (auth()->user()->can('product.request_price.create')) {
                            $sub->url(
                                action('ProductController@showPrices'),
                                "Báo giá",
                                ['icon' => 'fa fas fa-barcode', 'active' => request()->segment(1) == 'products' && request()->segment(2) == 'show-prices']
                            );
                        }
                        if (auth()->user()->can('product.create')) {
//                            $sub->url(
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('purchase.purchase_request.view')) {
                            $sub->url(
                                action('PurchaseRequestController@index'),
                                "Yêu cầu mua hàng",
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'purchases' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('purchase.purchase_request.create')) {
                            $sub->url(
                                action('PurchaseRequestController@create'),
            ...
            if (auth()->user()->can('sell.view') || auth()->user()->can('sell.create') || auth()->user()->can('direct_sell.access') ||  auth()->user()->can('view_own_sell_only')) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('sell.purchase_request.view')) {
                            $sub->url(
                                action('PurchaseRequestController@index'),
                                "Yêu cầu mua hàng",
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'sell' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('sell.purchase_request.create')) {
                            $sub->url(
                                action('PurchaseRequestController@create'),
                                "Tạo yêu cầu mua hàng",
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sell' && request()->segment(2) == "create"]
                            );
                        }
...
//            }
            ...
            if (in_array('stock', $enabled_modules) && (auth()->user()->can('stock.view') || auth()->user()->can('stock.create'))) {
                $menu->dropdown(
                    __('stock.stock'),
                    ...
                    function ($sub) use ($enabled_modules){
                        if (auth()->user()->can('stock.view')) {
                            $sub->url(
                                ...
                                __('stock.list'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'stock' && request()->segment(2) == null]
                            );
                        }

                        if (auth()->user()->can('stock.request_input.view')) {
                                $sub->url(
                                action('StockController@requestStock'),
                                "Yêu cầu nhập kho",
                                ['icon' => 'fa fas fa-book', 'active' => request()->segment(1) == 'request-stock' && request()->segment(2) == null]
                            );
                        }
...
//                        }
                        ...
                        if (auth()->user()->can('purchase_request.view')) {
                            $sub->url(
                                action('PurchaseRequestController@index'),
                                "Yêu cầu mua hàng",
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'purchases-request' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('purchase_request.create')) {
                            $sub->url(
                                action('PurchaseRequestController@create'),
                                "Tạo yêu cầu mua hàng",
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchases-request' && request()->segment(2) == "create"]
                            );
                        }

                        if (auth()->user()->can('stock.request_input.view')) {
                            $sub->url(
                                action('StockController@receiptPurchase'),
                                "Phiếu nhập kho",
                                ['icon' => 'fa fas fa-file-excel', 'active' => request()->segment(1) == 'stock' && request()->segment(2) == 'receipt--purchase']
                            );
                            $sub->url(
                                action('StockController@receiptSell'),
                                "Phiếu xuất kho",
                                ['icon' => 'fa fas fa-file-prescription', 'active' => request()->segment(1) == 'stock' && request()->segment(2) == 'receipt-sell']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-database']
...
//
            ...
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
                    __('lang_v1.payment_accounts_company'),
                    function ($sub) {
                        $sub->url(
                            action('AccountController@index'),
                            __('lang_v1.list_accounts_payment'),
                        ...
                        $sub->url(
                            action('AccountReportsController@payPurchase'),
                            "Phiếu chi mua hàng",
                            ['icon' => 'fa fas fa-file-excel', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'pay-purchase']
                        );
                        $sub->url(
                            action('AccountReportsController@paySell'),
                            "Phiếu thu bán hàng",
                            ['icon' => 'fa fas fa-file-prescription', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'pay-sell']
                        );
                    },
